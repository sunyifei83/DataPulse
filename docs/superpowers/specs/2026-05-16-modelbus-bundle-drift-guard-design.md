...
- **Q1: ~~Schema name verification.~~ RESOLVED 2026-05-16.** All 4 DP-expected schema strings are real names MB emits. MB published JSON Schema documents for 2/4 (`consumer_bundle_manifest.v1`, `release_status.v1`); the other 2 (`consumer_surface_admission.v1`, `consumer_bridge_config.v1`) exist only as payload tags in `scripts/ci/build_consumer_bundle.py:158,207`. Design handles via Layer 2a (mirror) + Layer 2b (DP-authored consumer contract).
- **Q5: MB schema publication gap.** Recommend MB publish JSON Schema documents for `modelbus.consumer_surface_admission.v1` and `modelbus.consumer_bridge_config.v1`. Until then, DP runs on consumer-driven contracts (Layer 2b). When MB publishes, switch those contracts from `consumer-contract/` to `upstream/` mirror. Fold this ask into issue #9 reply.
- **Q2: `source_pin` adoption.** Issue #9 reply proposes MB add `source_pin: <mb_commit_sha>` to `bundle_manifest.json`. If accepted, Layer 3's commit-distance check becomes precise instead of date-only.
