...
- `uv run scripts/datapulse_local_smoke.sh`
  - PASS=10 FAIL=1（FAIL 为平台能力/依赖边界：`wechat`、`rss`；RUN_ID `20260304_120313`）
  - 标准化基线（历史）：`PLATFORMS=twitter reddit youtube bilibili telegram xhs`，`PASS=11 FAIL=0`（RUN_ID `20260304_001737`）
  - 标准化基线（最新）：`PLATFORMS=twitter reddit youtube bilibili telegram xhs`，`PASS=11 FAIL=0`（RUN_ID `20260304_120341`）
